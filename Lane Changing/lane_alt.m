function [u_tot,u,b_mu1,b_mu2,bCol,b_final,b1_deriv_test] = ...
    lane_alt(t,x,options,velParam,hypParam,t_ini)
...
a4 = a4 + 1.0;
g4 = a3*exp(b3*(t-t_ini)+c3)+(d3+0.2);
...
A = den*[0;
    ...
    exp(-b_mu1)*(-d1v);
    0];
b = den*(exp(-b_mu1)*(-t1) + exp(-b_mu2)*(-t2)) ...
    + gamma*b_final;

Q = [1 0;0 1];
g = [4*(0.5*(tanh(v/3-1.5)+1)); 0];
lb = [0; -sat_steer];
