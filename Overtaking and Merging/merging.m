function [u_tot,b_mu1,b_mu2,b_col,b_col_tp,b_final1,b1_deriv_test] = ...
    merging(t,x,options,velParam,hypParam)
...
g1 = a1*t + b1;
b_mu1 = k1*(g1^2 - (xp-x1 - 10.55)^2);
d1xi = k1*(2*(xp-x1-12.55));
d1xp = k1*(-2*(xp-x1-12.55));
t1 = k1*(2*g1*a1);
...
% b_final1 = -log(exp(-b_col)+exp(-b_col_tp));
b_final1 = -log(exp(-b_mu2)+exp(-b_col));
b_final2 = -log(exp(-b_mu1)+exp(-b_col_tp));
...
% den = -1/(exp(-b_col)+exp(-b_col_tp));
% 
% A = den*[exp(-b_col)*(-d3x) + exp(-b_col_tp)*(-d4xi);...
%     exp(-b_col)*(-d3y) + exp(-b_col_tp)*(-d4yi)];
% 
% b = den*(exp(-b_col)*(-t3) + exp(-b_col_tp)*(-t4)) ...
%     + gamma*b_final1;

den = -1/(exp(-b_mu2)+exp(-b_col));
den2 = -1/(exp(-b_mu1)+exp(-b_col_tp));

A = [den*(exp(-b_col)*(-d3x)), den2*(exp(-b_mu1)*(-d1xi)+exp(-b_col_tp)*(-d4xi));...
    den*(exp(-b_mu2)*(-d2) + exp(-b_col)*(-d3y)), (d4yi)];

b = [den*(exp(-b_mu2)*(-t2) + exp(-b_col)*(-t3)) + gamma*b_final1;
     0.05*den2*(exp(-b_mu1)*(-t1) + exp(-b_col_tp)*(-t4) + gamma*2*b_final2)];
...
A_2 = (-den2)*[exp(-b_mu1)*(-d1xp) + exp(-b_col_tp)*(-d4xp); 
    -d4yp];
b_2 = 0.95*den2*(exp(-b_mu1)*(-t1) + exp(-b_col_tp)*(-t4) + gamma*2*b_final2);
